...
const BrokerForm = ({ initialData }: any) => {
  ...
  const OnSubmit: SubmitErrorHandler<FieldValues> = async (data) => {
    ...
    try {
      if(initialData){
        await axios.patch(`/api/brokers/${initialData?._id}`, data);
      }else{
        await axios.post(`/api/brokers`, data);
      }
      toast.success(`Broker ${actionsTitle} Successfully`);
      router.push("/brokers");
      router.refresh();
    } catch (err) {
      console.log(err);
